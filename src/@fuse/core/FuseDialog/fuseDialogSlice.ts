...
import { useFuseDialog, selectFuseDialogState, selectFuseDialogProps } from './FuseDialogContext';

// Legacy action creators (now use context methods directly)
export const openDialog = (payload: { children: React.ReactNode }) => {
	console.error('openDialog action is deprecated. Use useFuseDialog().openDialog() instead');
	return { type: 'DEPRECATED', payload };
};

export const closeDialog = () => {
	console.error('closeDialog action is deprecated. Use useFuseDialog().closeDialog() instead');
	return { type: 'DEPRECATED' };
};

// Re-export selectors and hooks
export { useFuseDialog, selectFuseDialogState, selectFuseDialogProps };
