...
const Card = forwardRef<HTMLDivElement, CardProps>(({
  ...
  return (
    ...
    >
      {/* Media Section */}
      {media && (
        <div className="card-media overflow-hidden -mt-4 -mx-4 mb-4 first:mt-0">
          {media}
        </div>
      )}
      ...
      {/* Actions Section */}
      {actions && (
        <div className="card-actions mt-4 first:mt-0 flex gap-2 justify-end">
          {actions}
        </div>
      )}
...
export const CardHeader = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('card-header space-y-1.5', className)}
      {...props}
    />
  )
);
